...
│   └── cache.service.ts
│
├── queue/                           # Bull Job Queue (Bull 작업 큐) — Optional, for scaling (선택, 서비스 확장 시 검토)
│   ├── queue.module.ts
│   └── processors/
│       ├── email.processor.ts
...
```
docker/
├── docker-compose.dev.yml           # Development Environment (개발 환경) - DB, Redis (Optional), etc. (DB, Redis (선택) 등)
│
