...
| Redis | 7.x | Cache, session, queue (캐시, 세션, 큐) — Optional, for scaling (선택, 서비스 확장 시 검토) |
| Bull | 4.x | Job queue (작업 큐) — Optional, for scaling (선택, 서비스 확장 시 검토) |
...
| Redis | 6379 | Cache/Session (캐시/세션) — Optional (선택) |
...
services:
  ...
  redis:      # Redis 7 — Optional, for scaling (선택, 서비스 확장 시 검토)
```
