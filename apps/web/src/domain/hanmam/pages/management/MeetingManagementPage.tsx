...
export default function MeetingManagementPage() {
  ...
  return (
    <div className="space-y-4 p-6">
      <h1 className="text-xl font-bold text-gray-800">{t('hanmam:menu.management.meetings')}</h1>
      <MockSearchFilter filters={[
        ...
        { type: 'date-range', label: t('hanmam:column.reportDate') },
      ]} />
      <MockDataGrid
        columns={[
          { key: 'no', label: t('hanmam:column.no'), width: '50px', align: 'center' },
          { key: 'title', label: t('hanmam:column.title') },
          { key: 'reportDate', label: t('hanmam:column.reportDate'), width: '120px' },
          { key: 'regDate', label: t('hanmam:column.regDate'), width: '120px' },
