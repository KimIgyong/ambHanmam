...
interface OtRatesTabProps {
  isAdmin: boolean;
}

export default function OtRatesTab({ isAdmin }: OtRatesTabProps) {
  ...
  const filtered = useMemo(
    () => params.filter((p) => p.paramKey.includes('OT_RATE')),
    [params],
  );
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              ...
              <th className="px-3 py-2">{t('hr:settings.otType')}</th>
              <th className="px-3 py-2">{t('hr:settings.multiplier')}</th>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('hr:settings.otType')}
                className="w-full rounded-lg border border-gray-300 px-3 py-1.5 text-sm focus:border-teal-500 focus:outline-none"
              />
            </div>
