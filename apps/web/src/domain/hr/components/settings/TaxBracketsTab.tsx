...
interface TaxBracketsTabProps {
  isAdmin: boolean;
}

export default function TaxBracketsTab({ isAdmin }: TaxBracketsTabProps) {
  ...
  const filtered = useMemo(
    () => params.filter((p) => p.paramKey.includes('PIT_')),
    [params],
  );
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              ...
              <th className="px-3 py-2">{t('hr:settings.bracketCeiling')}</th>
              <th className="px-3 py-2">{t('hr:settings.taxRate')}</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map((p) => {
              const isRate = p.paramKey.includes('RATE');
              return (
                <tr key={p.paramId}>
                  ...
                  <td className="px-3 py-2 text-gray-900">
                    {!isRate ? Number(p.paramValue).toLocaleString('vi-VN') : '-'}
                  </td>
                  <td className="px-3 py-2 text-gray-900">
                    {isRate ? `${Number(p.paramValue).toLocaleString('vi-VN')}%` : '-'}
                  </td>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                required
                placeholder="PIT_BRACKET_1_CEILING"
