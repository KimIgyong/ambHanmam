...
export default function PartnerRegisterPage() {
  ...
  const [invitation, setInvitation] = useState<{
    email: string; partnerName?: string; partnerCode?: string; role?: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    partnerPortalApiService.verifyInvitation(token)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await partnerPortalApiService.register({
        token, name, password,
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          ...
          <Link to="/partner/login" className="mt-4 inline-block text-sm font-medium text-emerald-600 hover:text-emerald-500">
            {t('register.login')}
          </Link>
        </div>
      </div>
      ...
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          {invitation.partnerName && (
            <p className="mt-2 text-sm font-medium text-emerald-600">{invitation.partnerName}</p>
          )}
        </div>
        ...
        <p className="mt-4 text-center text-sm text-gray-500">
          ...
          <Link to="/partner/login" className="font-medium text-emerald-600 hover:text-emerald-500">
            {t('register.login')}
          </Link>
        </p>
      </div>
