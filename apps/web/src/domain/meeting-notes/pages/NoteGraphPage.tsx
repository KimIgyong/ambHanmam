...
import { meetingNoteService, GraphData } from '../service/meeting-note.service';
...
export default function NoteGraphPage() {
  ...
  const [visibleTypes, setVisibleTypes] = useState<Set<string>>(new Set(['NOTE', 'TASK', 'ISSUE', 'PROJECT', 'MISSION']));
  ...
  useEffect(() => {
    ...
    setLoading(true);
    meetingNoteService
      .getGraphData({ scope, center_id: centerId })
      .then((data: GraphData) => {
        if (cancelled) return;
  ...
  const handleNodeClick = useCallback(
    (node: any) => {
      if (node.type === 'NOTE') {
        navigate(`/meeting-notes/${node.id}`);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">{t('meetingNotes:graph.title')}</h1>
        </div>
      ...
      {showFilters && (
        <div className="flex items-center gap-3 border-b border-gray-100 bg-gray-50 px-4 py-2">
          ...
          <span className="ml-auto text-xs text-gray-400">
            {filteredData.nodes.length} {t('meetingNotes:graph.nodes')} · {filteredData.links.length} {t('meetingNotes:graph.edges')}
          </span>
        </div>
      )}
      ...
      <div ref={containerRef} className="flex-1 bg-gray-900 relative">
        {loading ? (
          ...
          <div className="flex h-full items-center justify-center">
            <div className="text-gray-400">{t('meetingNotes:graph.noData')}</div>
          </div>
        ) : (
          <Suspense fallback={<div className="flex h-full items-center justify-center text-gray-400">{t('common:loading')}</div>}>
            <ForceGraph2D
