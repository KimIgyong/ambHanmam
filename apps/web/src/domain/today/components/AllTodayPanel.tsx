...
import { Users, LayoutGrid, List, Loader2, MessageCircle, ExternalLink, AlertTriangle, Target, EyeOff, Eye } from 'lucide-react';
import { useAllToday, useToggleMemberHidden } from '../hooks/useMyToday';
...
import type { MemberTodaySummary, MemberTodoItem } from '../service/today.service';

export default function AllTodayPanel() {
  const { t } = useTranslation('today');
  const { data, isLoading } = useAllToday();
  ...
  const departments = useMemo(() => {
    ...
    data.members.forEach((m) => { if (m.department) depts.add(m.department); });
    return [...depts].sort();
  }, [data]);
  ...
  if (!data || data.members.length === 0) {
    return <p className="py-10 text-center text-sm text-gray-400">{t('all.noMembers')}</p>;
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {departments.length > 0 && (
            ...
            >
              <option value="">{t('all.allDepts')}</option>
              {departments.map((d) => (
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b bg-gray-50 text-xs text-gray-500">
              <tr>
                ...
                <th className="px-4 py-2">{t('all.filterByDept')}</th>
            ...
            <tbody className="divide-y">
              {filteredMembers.map((m) => (
                <tr key={m.userId} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-gray-500">{m.unitName || m.department || '-'}</td>
                  <td className="px-4 py-2 text-center">{(m.todayDueCount + m.overdueCount) || '-'}</td>
                  <td className="px-4 py-2 text-center">
                    {m.overdueCount > 0 ? (
                      <span className="text-red-600">{m.overdueCount}</span>
                    ) : '-'}
                  </td>
