...
import {
  ...
  Search, AlertTriangle, List, Download,
} from 'lucide-react';
...
const importedApi = {
  getIssues: (params: Record<string, string | number>) =>
    apiClient.get('/issues/redmine-migration/imported-issues', { params }).then((r) => r.data.data),
  deleteIssues: (issueIds: string[]) =>
    apiClient.delete('/issues/redmine-migration/imported-issues', { data: { issue_ids: issueIds } }).then((r) => r.data.data),
  reimportIssues: (issueIds: string[], targetProjectId?: string, appId?: string) =>
    apiClient.post('/issues/redmine-migration/reimport', {
      issue_ids: issueIds,
...
export default function EntityRedmineImportedPage() {
  ...
  const issuesQuery = useQuery({
    queryKey: ['entity-redmine', 'imported-issues', statusFilter, searchText, page],
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['entity-redmine', 'imported-issues'] });
      queryClient.invalidateQueries({ queryKey: ['entity-redmine', 'imported-ids'] });
    },
  });
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">
                {t('externalTask:importedIssues.title')}
              </h1>
              <p className="text-sm text-gray-500">
                {t('externalTask:importedIssues.subtitle')}
              </p>
            </div>
          </div>
          <Link
            to="/issues/redmine-migration"
          ...
          >
            ...
            {t('externalTask:importedIssues.goToImport')}
          </Link>
        </div>
        ...
        <div className="mb-4 rounded-xl border border-gray-200 bg-white p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <option value="">{t('externalTask:importedIssues.allStatuses')}</option>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('externalTask:importedIssues.searchPlaceholder')}
                value={searchText}
            ...
            >
              ...
              {t('externalTask:importedIssues.refresh')}
            </button>
          </div>
        </div>
        ...
        {selectedIds.size > 0 && !confirmAction && (
          <div className="mb-4 flex items-center gap-3 rounded-xl border border-indigo-200 bg-indigo-50/50 px-4 py-3">
            <span className="text-sm font-medium text-gray-700">
              {t('externalTask:importedIssues.selected', { count: selectedIds.size })}
            </span>
            <div className="ml-auto flex items-center gap-2">
              ...
              >
                ...
                {t('externalTask:importedIssues.reimport')}
              </button>
              <button
              ...
              >
                ...
                {t('externalTask:importedIssues.delete')}
              </button>
            </div>
          </div>
        )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-red-800">
                  {t('externalTask:importedIssues.deleteConfirm', { count: selectedIds.size })}
                </p>
                <p className="mt-1 text-xs text-red-600">
                  {t('externalTask:importedIssues.deleteWarning')}
                </p>
                <div className="mt-3 flex items-center gap-2">
                  ...
                  >
                    ...
                    {t('externalTask:importedIssues.confirmDelete')}
                  </button>
                  <button
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-indigo-800">
                  {t('externalTask:importedIssues.reimportConfirm', { count: selectedIds.size })}
                </p>
                <p className="mt-1 text-xs text-indigo-600">
                  {t('externalTask:importedIssues.reimportWarning')}
                </p>
                <div className="mt-3 flex flex-wrap items-end gap-3">
                  <div className="min-w-[250px]">
                    <label className="mb-1 block text-xs font-medium text-gray-600">
                      {t('externalTask:importedIssues.targetProject')}
                    </label>
                    <select
                    ...
                    >
                      <option value="">{t('externalTask:importedIssues.autoCreateProject')}</option>
                      {amaProjects.map((p) => (
                  ...
                  >
                    ...
                    {t('externalTask:importedIssues.confirmReimport')}
                  </button>
                  <button
                    onClick={() => setConfirmAction(null)}
                    className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50"
        ...
        {deleteMutation.isSuccess && (
          <div className="mb-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            ...
            {t('externalTask:importedIssues.deleteSuccess', { count: (deleteMutation.data as any)?.deleted || 0 })}
          </div>
        )}
        {reimportMutation.isSuccess && (
          <div className="mb-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            <CheckCircle className="mr-1.5 inline h-4 w-4" />
            {t('externalTask:importedIssues.reimportSuccess', { count: (reimportMutation.data as any)?.issues?.success || 0 })}
          </div>
        )}
        {(deleteMutation.isError || reimportMutation.isError) && (
          <div className="mb-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
            <XCircle className="mr-1.5 inline h-4 w-4" />
            {t('externalTask:importedIssues.actionError')}
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colType')}</th>
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colTitle')}</th>
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colProject')}</th>
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colAssignee')}</th>
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colStatus')}</th>
                <th className="px-3 py-3 text-left font-medium text-gray-600">{t('externalTask:importedIssues.colImportedAt')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {issuesQuery.isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-gray-400">
                    {t('externalTask:importedIssues.noIssues')}
                  </td>
                </tr>
              ) : (
                issues.map((issue) => (
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm text-gray-600">
            {t('externalTask:importedIssues.totalCount', { count: totalCount })}
          </span>
          {totalPages > 1 && (
