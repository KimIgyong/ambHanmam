...
export class SvcClientEntity {
  ...
  @Column({ name: 'cli_original_lang', length: 5, default: 'ko' })
  cliOriginalLang: string;

  @Column({ name: 'cli_ent_id', type: 'uuid', nullable: true })
  cliEntId: string;
  ...
  @JoinColumn({ name: 'cli_account_manager_id' })
  accountManager: UserEntity;

  @ManyToOne(() => HrEntityEntity)
  @JoinColumn({ name: 'cli_ent_id' })
